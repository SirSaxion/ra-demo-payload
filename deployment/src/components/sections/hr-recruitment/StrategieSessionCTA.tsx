...
interface StrategieSessionCTAProps {
  title?: string;
  subtitle?: string;
  ctaLabel?: string;
}
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        ...
        <div className="absolute inset-0 opacity-[0.06] [mask-image:radial-gradient(70%_70%_at_50%_45%,#161616,transparent)]">
          <svg className="h-full w-full" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="hr-strategycta-grid" width="40" height="40" patternUnits="userSpaceOnUse">
                <path d="M 40 0 L 0 0 0 40" fill="none" stroke="currentColor" strokeWidth="1" />
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#hr-strategycta-grid)" className="text-foreground" />
          </svg>
        </div>
        {/* Glow strip */}
        ...
        <div className="relative mx-auto overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-3)] p-6 shadow-sm">
          ...
          <div className="grid gap-6">
            <ul className="mx-auto max-w-2xl space-y-3 text-base text-[var(--color-text-secondary)]">
              {[
                "Gratis recruitment analyse van 30 minuten",
                "Analyse van jouw huidige recruitment uitdagingen",
                "Identificatie van snelle verbetermogelijkheden",
                "Employer branding strategieën voor jouw organisatie",
                "Concreet stappenplan voor talent attraction",
                "BONUS: Direct inzichten ter waarde van €1000"
              ].map((text) => (
            ...
            <div className="mx-auto w-full max-w-2xl">
              <Button
                ...
                aria-label="Claim gratis recruitment analyse"
              >
                {/* Shine sweep */}
                ...
                <span className="relative z-10 flex w-full items-center justify-center gap-2">
                  ...
                  <span>Claim je gratis recruitment analyse</span>
              ...
              <p className="mt-3 text-center text-sm text-[var(--color-text-muted)]">
                Waarde €1000 • Volledig vrijblijvend • Beperkte plaatsen voor serieuze HR-teams
              </p>
            </div>
          </div>
        </div>
