...
import { Check, Calendar, ArrowRight, Globe } from "lucide-react";

export interface InternationalStrategySessionCTAProps {
  title?: string
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        ...
        <div className="absolute inset-0 opacity-[0.06] [mask-image:radial-gradient(70%_70%_at_50%_45%,#161616,transparent)]">
          <svg className="h-full w-full" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="international-strategycta-grid" width="40" height="40" patternUnits="userSpaceOnUse">
                <path d="M 40 0 L 0 0 0 40" fill="none" stroke="currentColor" strokeWidth="1" />
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill="url(#international-strategycta-grid)" className="text-foreground" />
          </svg>
        </div>
        {/* Glow strip */}
            ...
            <div className="mx-auto w-full max-w-2xl">
              ...
              >
                ...
                <span className="relative z-10 flex w-full items-center justify-center gap-2">
                  <Globe className="h-4 w-4 -translate-y-[1px] transition-transform duration-300 ease-out group-hover:-translate-y-0.5 group-hover:rotate-12" />
                  <span>{ctaLabel}</span>
              ...
              <p className="mt-1 text-center text-xs text-[var(--color-text-muted)] flex items-center justify-center gap-1">
                <Globe className="h-3 w-3 text-[var(--brand-500)]" /> {bonusText}
              </p>
            </div>
          </div>
        </div>
