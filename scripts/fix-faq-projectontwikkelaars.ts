/**
 * Fix ONLY the FAQ section phoneNumber in Projectontwikkelaars page
 * Run with: pnpm tsx scripts/fix-faq-projectontwikkelaars.ts
 */
...
async function migrate() {
  ...
  try {
    ...
    const updatedNlBlocks = nlVersion.blocks?.map((block: any) => {
      if (block.blockType === 'projectontwikkelaarsFAQSection' && 'phoneNumber' in block) {
