/**
 * Fix ONLY the FAQ section phoneNumber in Makelaars page
 * Run with: pnpm tsx scripts/fix-faq-makelaars.ts
 */
...
async function migrate() {
  ...
  try {
    ...
    const updatedNlBlocks = nlVersion.blocks?.map((block: any) => {
      if (block.blockType === 'makelaarsFAQSection' && 'phoneNumber' in block) {
