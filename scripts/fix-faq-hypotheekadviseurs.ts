/**
 * Fix ONLY the FAQ section phoneNumber in Hypotheekadviseurs page
 * Run with: pnpm tsx scripts/fix-faq-hypotheekadviseurs.ts
 */
...
async function migrate() {
  ...
  try {
    ...
    const updatedNlBlocks = nlVersion.blocks?.map((block: any) => {
      if (block.blockType === 'hypotheekadviseursFAQSection' && 'phoneNumber' in block) {
